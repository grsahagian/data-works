...
x = x.drop_duplicates(subset='website', keep="first")
x["Campaign"] = campaign_abbv




# combine potentially overlapping df with new search,
df = pd.concat([master, x]).drop_duplicates(subset='business_name')

# remove duplicate entries in website column
df = df.drop_duplicates(subset='website')

# only include results for new search
df = df[df.Campaign == campaign_abbv]
del df['index']

df.to_excel("Pennsylvania_search.xlsx")
