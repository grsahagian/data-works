...
master = pd.read_excel("/Users/Graham/data-works/scraping-yellow-pages/leads_master_export.xlsx", engine="openpyxl")
master = master.drop_duplicates(subset='website', keep="first")

# current search (new york search 1)
ny1 = pd.DataFrame(frame)
ny1 = ny1.drop_duplicates(subset='website', keep="first")
ny1["Campaign"] = 'NY1'
...
df = pd.concat([master, ny1]).drop_duplicates(subset='business_name')

# remove duplicate entries
df = df.drop_duplicates(subset='website')

# only include results for new search
df = df[df.Campaign == 'NY1']
del df['index']
del df['Schenectady,NY']


# df.to_excel("New_York_search.xlsx")