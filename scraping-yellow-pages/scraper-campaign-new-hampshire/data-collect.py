...
master = pd.read_excel("/Users/Graham/data-works/scraping-yellow-pages/leads_master.xls")
master = master.drop_duplicates(subset='website', keep="first")

# current search (new hampshire search 1)
nh1 = pd.DataFrame(frame)
nh1 = nh1.drop_duplicates(subset='website', keep="first")
nh1["Campaign"] = 'NH1'



# combine potentially overlapping df with new search,
df = pd.concat([master, nh1]).drop_duplicates(subset='business_name')

# remove duplicate entries
df = df.drop_duplicates(subset='website')

# only include results for new search
df = df[df.Campaign == 'NH1']


df.to_excel("New_Hampshire_search.xlsx")